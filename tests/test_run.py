...
def test_run_records_non_scope_keep_alive_preflight_failure(
    mock_client,
    mock_store,
    mock_runtime_class,
    mock_spawn_keep_alive,
    ...
    mock_client.keep_alive_assignment.side_effect = ColabRequestError(
        "Service Unavailable",
        MagicMock(),
        response,
    ...
    assert result.exit_code == 0, result.output
    assert persisted["session"].keep_alive_consecutive_failures == 1
...
def test_run_logs_invocation_provenance(
    ...
    assert invocation["high_mem"] is False
