...
| `repro_json_jq_lifecycle/` | Live e2e (~60s): one continuous session lifecycle where every step is composed with `jq` against real `--json` output — `new` (`jq` extracts the generated session name), `exec` of a `sys.exit(0)` script, `status -s`, `sessions`, `exec-async` piped straight into a `log --tail --json --since-offset` poll loop whose offset bookkeeping is entirely `jq`-driven, `stop` (real teardown), a second idempotent `stop`, and a final `status -s` on the now-gone session asserting the `session_not_found` divergence. Complements `repro_json_output/`'s `python3 -c` parsing (modeling a caller *without* `jq`) with the `jq`-available case, and is the only integration test covering `new`/`stop`/`sessions`/`status --json`. |
...
| `repro_sync/` | Live e2e: `mighty-colab sync` replaces a destination from a gzip payload, preserves selected Git `HEAD` and staged/unstaged/untracked status with `--git-aware`, omits an unrelated committed blob, and releases only its own CPU assignment. |
