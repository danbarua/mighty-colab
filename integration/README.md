...
| `repro_exec_async/` | Live e2e (~60s): `mighty-colab exec-async` returns near-instantly for a ~10s script; `mighty-colab log -s <session> -f` streams its stdout live and in order; a concurrent `exec-async` on the same session is refused while the first is running; a fresh `exec-async` after completion is allowed (a stale pid doesn't block a restart); `--output-log <path>` redirects output to a caller-chosen, previously-nonexistent nested path, and `colab status` reports it. |
