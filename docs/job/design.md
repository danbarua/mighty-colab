---
2026-09-16: Live multi-hour GPU evidence for the untested proxy-refresh gap: three real `mighty-colab job apply --async` A100 runs from a separate agent's session (`labkit-assistant`) completed at 87, 88, and 96 minutes wall clock -- all past the ~60-minute token-expiry boundary this doc spent lines 318-422 discriminating -- with continuous successful polling, `workload: succeeded`, sha256-verified artifact offload, and `cleanup: released`. Not a dedicated verification run by this project, but direct evidence against the "untested on GPU past the token's first hour" gap. Also fixed a real bug caught by the same dogfooding: `Orchestrator.poll()` returned immediately on a terminal `result.json`, skipping the log-pull block gated behind the still-running branch -- any script output written between the last periodic pull and process exit was silently lost, not a rare race but the normal shape of the terminating tick. `cleanup()` now pulls `runner.log` once more, through the same transport, immediately before `client.unassign()` releases the VM (PR #67). Added [`docs/job/mcp.md`](mcp.md): the MCP resource/notification layer built on top of this supervisor (`job://<id>` terminal-only subscribe, `job://<id>/logs`, `jobs://running`/`jobs://done`, `resources/list_changed`), live-verified end to end including one real gap (a stale pre-existing connection never learns about a job created after it connected -- host-side reconnect problem, not a server defect). Filed [#68](https://github.com/danbarua/mighty-colab/issues/68) (staging/orchestration layer above `JobSpec` -- a consumer's hand-rolled spec generator reimplements checkpoint-resume chaining, milestone-artifact wiring, and URL pre-verification in userland) and [#69](https://github.com/danbarua/mighty-colab/issues/69) (implement real retry/resume now that #27 only rejects inactive values) against the "declared but inactive controls" gap below.
