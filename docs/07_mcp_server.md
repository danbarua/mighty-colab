...
2026-08-12: **Known gap, not yet addressed**: the CLI's `--json` flag (see `docs/01_session_management.md`/`docs/02_execution_and_interactive.md`'s 2026-08-12 entries) has no equivalent here. MCP dispatch invokes `Command.invoke(ctx)` directly (Behavior #4) and captures combined stdout/stderr as plain text (Behavior #5) — it never goes through `cli.py`'s argv-level `--json` parsing at all, so there's no flag to forward even in principle. Wiring structured output into MCP would mean setting `Tool.outputSchema`/returning `structuredContent` per tool, using the same Pydantic envelope family (`src/colab_cli/envelopes.py`) that already backs the CLI. Two decisions are already locked in for whenever this is built: it should be **always-on** (no opt-in parameter, unlike the CLI's `--json` flag) for `exec`/`run`/`exec-async`/`log`; and `outputSchema` needs a hand-shaped `type: object`-rooted JSON Schema per tool (the MCP spec requires the schema root to be an object type, which the envelope Pydantic models don't uniformly guarantee) rather than dumping the strict-validation models' schema directly. No CLI↔MCP contract tests exist yet either — blocked on this wiring existing first.
