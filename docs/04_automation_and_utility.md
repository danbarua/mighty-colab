---
log:
2026-08-12: `auth.py`'s credential-loading errors are now `--json`-aware. Previously a malformed/missing `-c/--client-oauth-config` file raised an uncaught `FileNotFoundError`/`json.JSONDecodeError` (bare Python traceback, even under `--json`), and missing/invalid ADC credentials called the Python builtin `exit()` (an untyped `SystemExit` no `except Exception` call site could catch). Replaced with three plain, catchable exceptions -- `AuthConfigNotFoundError`, `AuthConfigInvalidError`, `AuthenticationError` (all in `auth.py`) -- each carrying `envelope_reason`/`envelope_hint` class attributes read by a new top-level catch-all in `cli.py:main()` around the `app()` call. That catch-all is the first genuinely central exception handler in this codebase: Click's own `Command.main()` (verified against its source) only catches `ClickException`/`Abort`/EPIPE, so *any* exception escaping a command body -- not just auth's -- previously reached the user as a raw traceback instead of the `[colab] Error: ...` / JSON-envelope shape every other failure mode here produces. `--debug` bypasses it and re-raises the real traceback, for anyone debugging the CLI itself rather than using it. `common.py`'s `State.sync_sessions()` had one pre-existing `except SystemExit:` (its best-effort "auth failed, just report no sessions" branch for a machine with no local session store) that only worked for the old ADC path; broadened to `except (SystemExit, AuthenticationError):` so it still holds now that path raises a normal exception instead. New integration test `integration/repro_auth_config_json/test.sh` (happy/unhappy paths for `-c` + `--json`); unit tests in `tests/test_auth.py`, `tests/test_auth_adc.py`, `tests/test_resolution_logic.py`, `tests/test_json_flag.py`.
