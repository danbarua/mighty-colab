---
log:
2026-09-13: Fixed [#10](https://github.com/danbarua/mighty-colab/issues/10): `sessions` and `status` now expose per-session keep-alive health in human and JSON output. The persisted consecutive-failure count increments for every failed pre-flight or daemon ping and resets after success; the summary reports last-success age and a local retention risk. It deliberately does not emit a reclamation-imminent claim because Colab exposes no deadline and the repository has not measured one. One live OAuth CPU session reported `healthy`, zero failures, and `normal` risk in both output modes; after terminating that session's daemon, JSON reported `stopped` and `elevated`, and the assignment was then released.

2026-09-13: All assignment control-plane requests now have explicit connect/read deadlines. The GET and POST phases of `assign` and `unassign`, plus `list_assignments`, use the shared `(10, 30)` second Requests timeout. Timeout exceptions propagate instead of being translated into assignment loss. Known-answer tests cover both successful wire calls and stalled mutation POSTs; a live ADC CPU `new` and `stop` both completed successfully, after which no assignment remained.

2026-09-13: A proxy 401/404 no longer deletes a local session binding without server confirmation. `State.prune_session()` lists current assignments, refreshes the saved proxy token and URL when the endpoint still exists, retains the binding when the confirmation request fails, and removes local state only when absence is confirmed. `sync_sessions()` passes its existing absence proof instead of issuing a duplicate list request. Human output distinguishes confirmed loss from retained state; JSON uses `session_access_failed` when the binding remains. `run --keep` and `console` also track reconciliation through their `finally` blocks: confirmed pruning is never resurrected, while retained sessions clear `running` on the freshly stored object instead of overw…

2026-09-13: `status` now labels its busy/idle value `LAST-KNOWN-LOCAL`. The value comes from the local `SessionState.running` marker after backend assignment reconciliation; it does not query the VM and must not be interpreted as current kernel activity. Both plain-text and `--json` output use the same explicit label. Verified against a live CPU session in both output modes.
...
- **Source**: The local `SessionState.running` marker, after `sync_sessions()` reconciles whether the assignment still exists on the backend. The command does not query the VM or kernel for activity.
- **Output Contract**: Plain-text and `--json` status values are prefixed `LAST-KNOWN-LOCAL` (for example, `LAST-KNOWN-LOCAL IDLE` or `LAST-KNOWN-LOCAL BUSY (exec.py)`). `BUSY` means a CLI-started command recorded itself locally; `IDLE` means no such local marker exists. Neither value claims current remote execution state.
...
Colab can delete inactive VMs, so the CLI implements a background keep-alive mechanism. Colab does not expose a reclamation deadline, and this repository has not measured one.
...
- **Pre-flight (`colab new`, `colab run`, `colab adopt --keep-alive`, and `colab job apply`)**: Each workflow pings once before spawning its daemon. A 403 `SCOPE_NOT_PERMITTED` aborts; workflows that created the assignment unassign it, while `adopt` leaves its pre-existing assignment intact. Other failures are tolerated, persisted as the first consecutive failure, and retried by the daemon. A success records the first `last_keep_alive_ping` and resets the counter.
- **Health summary**: `sessions` and `status` show the same human/JSON state derived from local facts: `healthy` (alive daemon, last ping succeeded, no failures), `starting` (alive, no completed ping), `transient_failure` (one or two failures), `degraded` (three or more failures), `stopped` (configured daemon is dead), or `disabled` (no daemon configured). JSON fields are `keep_alive_health`, `keep_alive_consecutive_failures`, optional `keep_alive_last_success_age_seconds`, and `keep_alive_retention_risk` (`normal`, `elevated`, or `unknown`). `normal` requires an observed prior success. Orphaned server assignments omit these local-only fields. `elevated` means the local mechanism needs attention; it does not mean reclamation is known to be imminent.
- **Structured logging**: The daemon emits `keep_alive_started` (with `pid`, `endpoint`), one `keep_alive_error` per failed iteration (with `status_code`, `error_type`, truncated `error`, `response_body`, `iteration`, `consecutive_4xx`, and `consecutive_failures`), and `keep_alive_stopped` (with `reason`, `iterations`, `duration_seconds`, optional `last_error`, optional `expected_endpoint`/`actual_endpoint`). All three are rendered specially by `colab log` so users get diagnostic context without parsing JSONL by hand.
- **Termination**:
...
## Assignment Reconciliation and Proxy Credentials

`sync_sessions()` lists server assignments and prunes local entries whose endpoints are confirmed absent. Proxy 401/404 responses are not independent proof of assignment loss because `RuntimeProxyInfo.token` expires. Terminal-error handlers therefore call the same guarded `State.prune_session()` path: it lists assignments, replaces the persisted token and URL when the endpoint survives, and retains the binding when the list request is inconclusive. Only a successful list response that omits the endpoint permits deletion. A retained JSON failure uses `reason="session_access_failed"`; `reason="session_lost"` is reserved for confirmed absence.
