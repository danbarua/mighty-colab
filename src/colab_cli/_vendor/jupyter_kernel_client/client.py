...
""")
        ...
        # VENDOR PATCH: imported lazily. `jupyter-mimetypes` pulls pyarrow
        # (~35MB) and is only needed by set_variable/get_variable, which
        # this CLI never calls -- a module-level import would make every
        # install pay for it. See VENDOR.md.
        from jupyter_mimetypes import serialize_object
...
""")
        ...
        if results["status"] == "ok" and results["outputs"]:
            ...
            # VENDOR PATCH: lazy, see set_variable above. Imported OUTSIDE
            # the try below: that block only exists to wrap deserialization
            # failures in a RuntimeError, and catching a missing
            # jupyter-mimetypes install there would surface as the
            # misleading "No valid deserialization data found: No module
            # named 'jupyter_mimetypes'" instead of a clear ImportError.
            from jupyter_mimetypes import deserialize_object
